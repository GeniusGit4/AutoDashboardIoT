...
#define JERK_THRESHOLD     5.0f
#define LED_PIN            GPIO_PIN_13
#define LED_GPIO_PORT      GPIOD
#define LED_BLINK_INTERVAL 200  // ms
/* USER CODE END PD */
...
void MPU6050_Init(void)
{
    ...
    data = 0x07;  // Sample rate = 1kHz / (1 + 7) = 125Hz
...
void MPU6050_ReadAccel(void)
{
    HAL_I2C_Mem_Read(&hi2c2, MPU6050_ADDR, MPU6050_REG_ACCEL, I2C_MEMADD_SIZE_8BIT, rawData, 14, HAL_MAX_DELAY);
...
void CalculateJerkAndHandleAlert(void)
{
    ...
    if (fabs(jerkX) > JERK_THRESHOLD || fabs(jerkY) > JERK_THRESHOLD || fabs(jerkZ) > JERK_THRESHOLD)
    {
